...
/**
 * Atualiza todos as tentativas registrados no banco de dados
 * 
 ...
 * @returns Retorna status 404 se a tentativa não for encontrada.
 * @example
 * GET/api/tentativas
 * 
 * // Exemplo de resposta em caso de erro
 * Status: 404
 * Body: { "message": "tentativa não encontrada" } 
 */
...
/**
 * Exclui uma tentativa do banco de dados com base no ID fornecido.
 * 
 * @param req - Objeto da requisição (não utilizado neste endpoint).
 * @param context - Objeto contendo os parâmetros da rota, incluindo o ID da tentativa a ser deletada.
 * @returns status 204 em caso de sucesso
 * @returns status 400 para "ID inválido"
 * @returns status 404 para "tentativa não encontrada"
 * @returns status 500 para "erro interno ao deletar"
 * 
 * @example
 * DELETE/api/tentativa
 */
...
//  await authenticate(req);
  try {
    await prisma.tentativa.delete({ where: { id } 
    });
